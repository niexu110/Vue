...
Page({
    data: {
        id: "", uid: '', eid: '', token: '',
        list: {}, goods: {}, engagement: {}, count: "",//几人接受
        ...
        other: '', friend: '', userage: '', constellation: '', trustworthiness: '85', //小于30为低 大于30且小于50为中 大于50为高
        placeImg: '',
        promptText: '请双方在约会前提前沟通好，约会前2小时双方都可取消约会。已经冻结的额费用，返还支付账户，但取消方会扣除相应的约会信用值。',
        heOrShe: '联系他/她',
        onClickButoon: false, releaseImg: '', username: 'CHLOE', disabledRefuse: false, disabledAgree: false,
    ...
    exitApp: function () {
        var self = this;
        console.log(self.data.engagement.eid)
        wx.navigateTo({
            url: '../removeAppointments/removeAppointments?id='+self.data.engagement.id,
        })
    },
    //点击拒绝
    refuse: function () {
    ...
    onLoad: function (options) {
        ...
        wx.request({
            ...
            success: function (res) {
                ...
                engagement.mood_time = new Date(parseInt(engagement.mood_time) * 1000).toLocaleString();
                console.log(engagement.daojishi)
                //加载倒计时
                var the_s = engagement.daojishi;//定义剩余时间, 必须用时间戳.单位为秒
                setInterval(promote, 1000);//每秒执行一次下面的函数
                function promote() {
                    var d = Math.floor((the_s / 3600) / 24);
                    var g = Math.floor((the_s - d * 24 * 3600) / 3600) + d * 24;
                    var e = Math.floor((the_s - d * 24 * 3600 - g * 3600) / 60);
                    var f = (the_s - g * 3600) % 60;
                    var html = g + ':' + e + ':' + f;
                    the_s--;
                    self.setData({ daojishi: html })
                }
                self.setData({
