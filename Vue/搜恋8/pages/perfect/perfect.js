...
Page({
  ...
  dataForm:function(e){
      ...
      if(list.nickname==""){
          wx.showToast({ title: '昵称必须4到到8字符',icon: 'loading',duration: 2000 })
      }else if(list.birthday=="点击选择生日"){
         wx.showToast({ title: '生日不能为空',icon: 'loading', duration: 2000 })
      }else{
        wx.showToast({title: '正在提交....', icon: 'loading', duration: 2000 })
     wx.request({
      ...
      success: function(res){
        // success
        var url=config.SL_URL+'/index.php/Home/Article/images';
        var id=res.data.data;
        var object={
             uid:id,
             url:img,
             token:md5.MD5(id+'solianJSKASDKES')
        }
         if(res.data.code==404){
            wx.showModal({title: '提示', content: '该手机号已经注册过请直接登录',
           success: function(res) {
           if (res.confirm) {wx.redirectTo({ url: '../login/login',}) } } })
