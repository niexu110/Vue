    ...
    wx.request({
      ...
      success: function(res){
        ...
        for(var k in obj){
          ...
          obj[k].abort_time=new Date(parseInt(obj[k].abort_time) * 1000).toLocaleString().substr(0, 16).replace(/\//g, '-');
          obj[k].date_time= new Date(parseInt(obj[k].date_time) * 1000).toLocaleString().substr(0, 16).replace(/\//g, '-');  
